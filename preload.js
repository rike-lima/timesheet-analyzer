    ...
    try {
      ...
      for (let i = 0; i < allCells.length; i++) {
        ...
        if (
          ...
          if (nomeAtual) nomes.add(nomeAtual);
        }
      }
...
,
  ...
  try {
    const filePath = await window.api.escolherLocalSalvar(`relatorio-${mes}-${ano}.pdf`, 'pdf');
    if (!filePath) return false;

    const doc = new PDFDocument();
    doc.pipe(fs.createWriteStream(filePath));

    doc.fontSize(18).text(`Relatório de Horas Trabalhadas - ${mes}/${ano}`, { align: 'center' });
    doc.moveDown();
